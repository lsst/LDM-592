...
\setDocChangeRecord{%
\addtohist{1.0}{2017-11-13}{Initial version from Working Group internal spreadsheet.} {T.~Jenness}
}
...
\item
The butler expands the incomplete DatasetExpresion by setting the filter and selecting only those CCDs for which raw image data is available.
...
Note: this use case is intended to highlight the need for the butler to be able to expand an incomplete DatasetExpresion.
...
\begin{draftnote}
  \emph{Petravick:} Please give some indication of retention policies.
\end{draftnote}
...
\item[ConcreteDataset] The in-memory manifestation of a Dataset (e.g. an \texttt{afw::image::Exposure} instance with the contents of a particular \texttt{calexp}).

\item[DatasetType] The conceptual type of which Datasets are instances (e.g.\ \texttt{calexp}).  A new DatasetType can be defined by combining a DatasetMetatype, a DatasetIdentifierType, and a string name.

\item[DatasetMetatype] A category of DatasetTypes that utilize the same in-memory classes for their ConcreteDatasets and can be saved to the same file format(s), using the same low-level input and output code.
...
\item[Data Input System] A software agent that can be used to read a Dataset from storage into a ConcreteDataset in memory.

\item[Data Output System] A software agent that can be used to write a ConcreteDataset in memory to a Dataset in storage.
